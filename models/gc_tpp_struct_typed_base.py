...
# ---------- 2. Coarse Types（仅用于 per-coarse 分析） ----------
    ...
    print(f"[Struct-Typed-Base] Start Training ({mode_tag})... seed={seed} protocol={protocol}")
    # --- auto-dump split indices (Global Median baseline) ---\n    import os as _os, numpy as _np\n    if _os.environ.get('DUMP_SPLITS','0') == '1':\n        _need = ['train_idx','test_idx','seen_idx','ood_idx']\n        if all(k in locals() for k in _need):\n            _dm = locals().get('data_mode', locals().get('mode', None))\n            _pt = locals().get('protocol', None)\n            _sd = locals().get('seed', None)\n            # fallback: args or env\n            try:\n                _args = locals().get('args', None) or globals().get('args', None)\n                if _dm is None and _args is not None: _dm = getattr(_args,'data_mode', None)\n                if _pt is None and _args is not None: _pt = getattr(_args,'protocol', None)\n                if _sd is None and _args is not None: _sd = getattr(_args,'seed', None)\n            except Exception:\n                pass\n            if _dm is None: _dm = _os.environ.get('DATA','unknown_data')\n            if _pt is None: _pt = _os.environ.get('PROT','unknown_prot')\n            if _sd is None: _sd = 0\n            _os.makedirs('logs', exist_ok=True)\n            _out = f"logs/splits_{_dm}_{_pt}_seed{_sd}.npz"\n            _np.savez(_out,\n                     train_idx=_np.asarray(locals()['train_idx'], dtype=_np.int64),\n                     test_idx=_np.asarray(locals()['test_idx'], dtype=_np.int64),\n                     seen_idx=_np.asarray(locals()['seen_idx'], dtype=_np.int64),\n                     ood_idx=_np.asarray(locals()['ood_idx'], dtype=_np.int64))\n            print('[DUMP_SPLITS] saved ->', _out)\n            return\n        else:\n            print('[DUMP_SPLITS] missing vars:', [k for k in _need if k not in locals()])\n
    num_epochs = 15
    ...
    if _os.environ.get("DUMP_SPLITS","0") == "1":
        ...
        def _is_int_1d(a):
            return (a is not None) and getattr(a, "ndim", 0) == 1 and _np.issubdtype(a.dtype, _np.integer) and a.size > 0

        def _mask_to_idx(a):
            return _np.where(a)[0].astype(_np.int64)

        def _collect_from_obj(obj, prefix=""):
            out = {}
            # common subset/dataset attrs
            for attr in ["indices", "idx", "index"]:
                if hasattr(obj, attr):
                    a = _as_np(getattr(obj, attr))
                    if _is_int_1d(a):
                        out[prefix + attr] = a.astype(_np.int64)

            if isinstance(obj, dict):
                for k,v in obj.items():
                    a = _as_np(v)
                    kl = str(k).lower()
                    if _is_int_1d(a) and any(t in kl for t in ["idx","index","indices"]):
                        out[prefix + str(k)] = a.astype(_np.int64)
                    if (a is not None) and getattr(a,"ndim",0)==1 and a.dtype==_np.bool_ and any(t in kl for t in ["seen","ood","test"]):
                        out[prefix + str(k)] = _mask_to_idx(a)
            ...
            if hasattr(obj, "__dict__"):
                for k,v in obj.__dict__.items():
                    ...
                    kl = str(k).lower()
                    if _is_int_1d(a) and any(t in kl for t in ["idx","index","indices"]):
                        out[prefix + k] = a.astype(_np.int64)
                    if (a is not None) and getattr(a,"ndim",0)==1 and a.dtype==_np.bool_ and any(t in kl for t in ["seen","ood","test"]):
                        out[prefix + k] = _mask_to_idx(a)

            # dataloader.dataset / subset.dataset
            if hasattr(obj, "dataset"):
                out.update(_collect_from_obj(getattr(obj, "dataset"), prefix + "dataset."))
            return out
        ...
        for _k,_v in list(locals().items()):
            if _k.startswith("_"):
                continue
            got = _collect_from_obj(_v, "")
            for kk,vv in got.items():
                cand[f"local.{_k}.{kk}"] = vv

            a = _as_np(_v)
            kl = _k.lower()
            if _is_int_1d(a) and any(t in kl for t in ["train","test","seen","ood"]) and any(t in kl for t in ["idx","index","indices"]):
                cand["local."+_k] = a.astype(_np.int64)
            if (a is not None) and getattr(a,"ndim",0)==1 and a.dtype==_np.bool_ and any(t in kl for t in ["seen","ood","test"]):
                cand["local."+_k] = _mask_to_idx(a)
        ...
        train_idx = _pick(["train","idx"]) or _pick(["train","indices"]) or _pick(["train","index"])
        test_idx  = _pick(["test","idx"])  or _pick(["test","indices"])  or _pick(["test","index"])
        seen_idx  = _pick(["seen","idx"])  or _pick(["seen","indices"])  or _pick(["seen","index"])
        ood_idx   = _pick(["ood","idx"])   or _pick(["ood","indices"])   or _pick(["ood","index"])

        if any(x is None for x in [train_idx,test_idx,seen_idx,ood_idx]):
            print("[DUMP_SPLITS_V2] FAILED. candidates(top60):")
            for i,k in enumerate(sorted(cand.keys())[:60]):
                print("  -", k, "len=", len(cand[k]))
            raise SystemExit(2)

        _dm = locals().get("data_mode", locals().get("mode", None)) or _os.environ.get("DATA","unknown_data")
        _pt = locals().get("protocol", None) or _os.environ.get("PROT","unknown_prot")
        _sd = locals().get("seed", None)
        if _sd is None:
            try:
                _args = locals().get("args", None) or globals().get("args", None)
                if _args is not None:
                    _sd = getattr(_args, "seed", None)
            except Exception:
                pass
        if _sd is None:
            _sd = 0
        ...
        _out = f"logs/splits_{_dm}_{_pt}_seed{_sd}.npz"
        _np.savez(_out,
                 train_idx=_np.asarray(train_idx, dtype=_np.int64),
                 test_idx=_np.asarray(test_idx, dtype=_np.int64),
                 seen_idx=_np.asarray(seen_idx, dtype=_np.int64),
                 ood_idx=_np.asarray(ood_idx, dtype=_np.int64))
        print("[DUMP_SPLITS_V2] saved ->", _out, "sizes=", len(train_idx), len(test_idx), len(seen_idx), len(ood_idx))
        raise SystemExit(0)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if early_stopper.step(avg_val_nll):
            print(f"[Struct-Typed-Base] Early stopping at epoch {epoch}. Best Val NLL = {early_stopper.best:.4f}")
            break
    ...
    print(f"[Struct-Typed-Base] Test NLL  (all) = {avg_test_nll:.4f}")
    print(f"[Struct-Typed-Base] Test RMSE (log Δt, all) = {test_rmse:.4f}")
    print(f"[Struct-Typed-Base] Test MAE  (log Δt, all) = {test_mae:.4f}")
    print(f"[Struct-Typed-Base] Test Cov90 (all) = {test_cov90:.4f}")

    print(f"[Struct-Typed-Base] Test NLL  (Seen) = {avg_test_nll_seen:.4f} | count={test_count_seen}")
    print(f"[Struct-Typed-Base] Test RMSE (Seen) = {test_rmse_seen:.4f} | count={test_count_seen}")
    print(f"[Struct-Typed-Base] Test MAE  (Seen) = {test_mae_seen:.4f} | count={test_count_seen}")
    print(f"[Struct-Typed-Base] Test Cov90 (Seen) = {test_cov90_seen:.4f} | count={test_count_seen}")

    print(f"[Struct-Typed-Base] Test NLL  (OOD)  = {avg_test_nll_ood:.4f} | count={test_count_ood}")
    print(f"[Struct-Typed-Base] Test RMSE (OOD)  = {test_rmse_ood:.4f} | count={test_count_ood}")
    print(f"[Struct-Typed-Base] Test MAE  (OOD)  = {test_mae_ood:.4f} | count={test_count_ood}")
    print(f"[Struct-Typed-Base] Test Cov90 (OOD) = {test_cov90_ood:.4f} | count={test_count_ood}")

    # ---------- 保存 per-coarse 指标 CSV ----------
    os.makedirs("logs", exist_ok=True)
    coarse_csv_path = f"logs/gc_tpp_struct_typed_base_coarse_{data_mode}.csv"
    ...
    print(f"[Struct-Typed-Base] Saved coarse-level metrics to {coarse_csv_path}")
    ...
    print(f"[Struct-Typed-Base] Wrote metrics.json -> {metrics_path}")

    # ---------- 保存 npz ----------
    if data_mode == "toy":
        save_path = "logs/gc_tpp_struct_typed_base_toy.npz"
    elif data_mode == "icews_real":
        save_path = "logs/gc_tpp_struct_typed_base_icews_real.npz"
    else:
        save_path = f"logs/gc_tpp_struct_typed_base_{data_mode}.npz"
    ...
    print(f"[Struct-Typed-Base] Saved curves to {save_path}")
