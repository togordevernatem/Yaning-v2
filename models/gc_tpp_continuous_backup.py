...
class GCTPPContinuous(nn.Module):
    ...
    def forward(self,
        ...
        # 图结构编码（这里 Core 只用图级 g_t，不用节点级 H）
        g_t = self.graph_encoder(X_snapshots, edge_index, return_node_repr=False)
...
# ===========================
# 6. 训练 & 验证 & 测试 主流程（含 RMSE/MAE + EarlyStopping + 曲线保存）
# ===========================
def run_gc_tpp_continuous(data_mode: str = "toy"):
    ...
    print(f"[INFO] Test RMSE (Δt prediction)       = {test_rmse:.4f}")
    print(f"[INFO] Test MAE  (Δt prediction)       = {test_mae:.4f}")
    ...
    save_path = os.path.join("logs", f"gc_tpp_core_{suffix}.npz")
