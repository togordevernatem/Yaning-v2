...
def main():
    ...
    parser.add_argument(
        ...
        choices=[
            ...
            "gc_tpp_struct",
            "gc_tpp_struct_typed",
            "gc_tpp_struct_typed_base",
        ],
        help=(
            "选择要运行的模型："
            "gc_tpp_continuous（Core 连续版） / "
            "gc_tpp_struct（Struct 结构版） / "
            "gc_tpp_struct_typed（Struct-Typed + Profile 版） / "
            "gc_tpp_struct_typed_base（Struct-Typed 无 Profile）"
        ),
    )
    ...
    parser.add_argument("--protocol", type=str, default="protB", choices=["protA", "protB"],
                        help="evaluation protocol: protA (temporal split) or protB (mixed split)")
    parser.add_argument("--out_dir", type=str, default="logs/table_runs",
